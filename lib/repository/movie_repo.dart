...
class MovieRepository {
  ...
  Future<List<Movie>> fetchPopularMovies() async {
    final String url = '${AppConfig.baseUrl}movie/popular?api_key=${AppConfig.apiKey}';
  ...
  Future<List<Movie>> fetchUpcomingMovies() async {
    final String url =
        '${AppConfig.baseUrl}movie/upcoming?api_key=${AppConfig.apiKey}&language=en-US&page=1';
    try {
      final response = await http.get(Uri.parse(url));
